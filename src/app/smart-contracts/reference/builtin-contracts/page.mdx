...
Updates the properties or trigger functionalities of a fungible token, NFT collection, or a single NFT. See all types [here](/contracts).

- **Mint:** Directly mints assets in the target account.

```rust
    self.send.kda_mint(token, nonce, amount)
    //or
    self.send.kda_mint_with_address(token, nonce, amount, address);
```
