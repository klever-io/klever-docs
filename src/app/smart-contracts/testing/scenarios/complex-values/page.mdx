...
In the previous section, we discussed representations of simple types [here](/developers/scenario-reference/values-simple). These representations are suitable for arguments of types like `usize`, `BigUint`, or `&[u8]`. However, there is a need to specify complex types such as custom structs or lists of items.
...
Please note that the pipe operator only takes care of the concatenation itself. You are responsible for ensuring that [nested encoding](/developers/data/serialization-overview/#the-concept-of-top-level-vs-nested-objects) is used where appropriate.
