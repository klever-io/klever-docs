...
> **Tip:** Before setting up your node, use the [Benchmark Tool](/benchmark-tool) to verify that your machine meets the performance requirements to run a validator node.
...
- A CPU with 4 cores.
  - **x86_64 systems** (Intel/AMD): Must support **AVX** or **SSE 4.2** instruction sets for smart contract execution.
  - **ARM systems** (Apple Silicon, ARM servers): Use native ARM64 builds.
- A RAM with 8GB.
- An SSD with 200 GB.
- An internet connection speed of at least 100 Mbps.
- Linux or MacOS.

The Klever node requires **AVX** or **SSE 4.2** CPU instructions for smart contract execution. If missing, you'll see:

```
TRACE[...] error=could not create wasmer instance: The target x86_64 without AVX or SSE 4.2 is not yet supported
```
...
uname -m  # Returns: x86_64 (Intel/AMD) or arm64 (Apple Silicon or other ARM64 systems)
```

**For x86_64 systems only:**

```bash
# Linux - Check for AVX/SSE 4.2
grep -o '\(avx\|sse4_2\)' /proc/cpuinfo | sort -u

# macOS Intel - Check for AVX/SSE 4.2
sysctl hw.optional.sse4_2 hw.optional.avx1_0
```

**For ARM systems (Apple Silicon M1/M2/M3/M4):**
- These CPU feature checks are not applicable to ARM.
- You must use native ARM64 builds of Klever node.
- No additional verification is required beyond confirming ARM architecture.

**Common issues:**

- **VMs (KVM/VMware/VirtualBox):** Enable CPU pass-through in hypervisor settings (host-passthrough for KVM, enable VT-x/AMD-V for VMware/VirtualBox)
- **Cloud instances:** Use instance types with full CPU support (AWS: T3+, GCP: N2/N2D, Azure: Dv4/Ev4+)
- **Docker containers:** Use host CPU features — verify the host system meets AVX/SSE 4.2 requirements
- **Apple Silicon (M1/M2/M3/M4):** Use native ARM64 builds, not x86_64 emulation
- **Older hardware:** Requires Intel Sandy Bridge (2011+) or AMD Bulldozer (2011+)

<Note color="yellow">
    **Impact:** Without AVX/SSE 4.2, smart contracts will NOT work. Validators cannot participate in consensus for blocks with smart contracts. Observer nodes can sync but cannot execute/validate smart contracts.
</Note>
...
2. **Start your node** with the external.yaml configuration properly set with Elasticsearch connector enabled
3. **Verify the setup** by checking for the confirmation message: "Node is running with a valid indexer"
...
1. **Geographic Distribution**: Deploy backup nodes in different locations for maximum availability
2. **Network Connectivity**: Ensure all nodes have reliable internet connections
3. **Regular Monitoring**: Monitor all nodes to ensure they're ready to take over when needed
4. **Database Backups**: Keep database backups synchronized between main and backup nodes
